...
async def _handle_connect_phone(message, user_id, phone_text):
    ...
    await message.reply_text(
        ...
        "Send it as `FLOW 12345` (replace 12345 with your code).\n\n"
...
async def _handle_connect_code_or_password(message, user_id, text):
    ...
    try:
        ...
        if stage == "code":
            ...
            try:
                valid, code_or_error = user_sessions.extract_otp_from_command(text)
                if not valid:
                    await message.reply_text(f"❌ {code_or_error}")
                    return
                await user_sessions.submit_code(user_id, code_or_error)
