...
from database.db import get_connection, mark_telegram_session_reconnect_required
...
async def start_owner_engine(owner_id):
    ...
    async with _engines_lock:
        ...
        try:
            ...
            # This includes a changed encryption key or a tampered ciphertext.
            # Do not leave it eligible for the shared engine after restart.
            mark_telegram_session_reconnect_required(owner_id)
            logger.warning("Owner %s's stored session cannot be decrypted; reconnect required.", owner_id)
            return
        ...
        try:
            ...
            if not await client.is_user_authorized():
                mark_telegram_session_reconnect_required(owner_id)
                logger.warning("Owner %s's stored session is no longer authorized; reconnect required.", owner_id)
