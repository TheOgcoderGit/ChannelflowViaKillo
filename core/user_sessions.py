...
from database.db import get_connection, verify_telegram_session_reconnect_identity
...
# ---- OTP input-format handling (FLOW prefix) --------------------------
# Telegram's anti-abuse systems are NOT touched: the OTP digits sent to
# Telethon are exactly what Telegram delivered. The FLOW prefix is a
...
def extract_otp_from_command(text: str):
    """Parses ``FLOW12345`` or ``FLOW 12345`` into ``12345``.

    Returns (ok, code_or_error_message). Validates:
      * message starts with exactly FLOW (case-insensitive)
    ...
    if not normalized.upper().startswith(OTP_COMMAND_PREFIX):
        return False, (
            f"Send the code in this format: {OTP_COMMAND_PREFIX} <code>\n"
            f"Example: {OTP_COMMAND_PREFIX} 12345"
        )

    code = normalized[len(OTP_COMMAND_PREFIX):].strip()
...
async def submit_code(user_id: int, code: str):
    ...
    extract_otp_from_command) - the FLOW prefix never reaches this
...
async def _finalize(user_id):
    ...
    # The external Telegram identity must be claimed from get_me(), not
    # inferred from the ChannelFlow bot user who initiated the connection.
    try:
        me = await client.get_me()
        external_telegram_id = me.id
    except Exception:
        await client.disconnect()
        raise ConnectError("Telegram login could not be completed. Please start /connect again.")
    ...
    try:
        # Serialize a reconnect with any explicit disconnect or concurrent
        # login for this owner. A session marked reconnect_required retains
        # its external-account claim, so it may only be replaced after the
        # user has authenticated that same Telegram account again.
        cur.execute("BEGIN IMMEDIATE")
        try:
            verify_telegram_session_reconnect_identity(cur, user_id, external_telegram_id)
        except ValueError:
            raise ConnectError(
                "This connection needs to be restored with the Telegram account "
                "previously linked to ChannelFlow. Disconnect it first to use a different account."
            )

        cur.execute(
            """
            INSERT INTO user_telegram_sessions(
                telegram_id, external_telegram_id, encrypted_session, phone_number, status
            )
            VALUES (?, ?, ?, ?, 'connected')
            ...
            """,
            (user_id, external_telegram_id, encrypted, phone_number),
        )
        conn.commit()
    except ConnectError:
        conn.rollback()
        ...
        conn.rollback()
        # The partial unique index makes this branch race-safe.  Do not expose
        # the account or its current owner to the requesting user.
        if "idx_active_external_telegram_owner" in str(exc) or "external_telegram_id" in str(exc):
            raise ConnectError(
                "This Telegram account is already connected to another ChannelFlow account."
            )
        raise
    finally:
        conn.close()
...
def cancel_connect(user_id):
    """Cancel an attempt and promptly close its temporary client.

    This function is synchronous because the handler state-reset helper is
    shared by several flows.  It schedules the actual async disconnect on the
    application's running loop rather than dropping the reference and leaking
    an open MTProto connection until process exit.
    """
    if user_id not in _pending:
        return

    try:
        asyncio.get_running_loop().create_task(_drop_pending(user_id))
    except RuntimeError:
        # There is no running event loop only in unusual synchronous teardown
        # code. Remove state so it cannot be used; normal bot operation always
        # takes the branch above and disconnects immediately.
