...
# The PRD specifies the FLOW<code> format as the primary flow.
# The old /myflow prefix is deprecated but still accepted for backward
# compatibility until users migrate.
FLOW_COMMAND_PREFIX = "/flow"

# Legacy support: /myflow<code> still accepted but /flow<code> is primary
LEGACY_OTP_COMMAND_PREFIX = "/myflow"


def extract_otp_from_flow_command(text: str):
    """Parses 'FLOW12345' or 'FLOW 12345' -> '12345'.

    Returns (ok, code_or_error_message). Validates:
      * message starts with exactly FLOW (optionally preceded by /)
    ...
    # Try FLOW<code> format (primary, PRD-mandated)
    if t.upper().startswith("FLOW"):
        code = t[4:].strip()
        # Handle "FLOW 12345" format with space
        if not code:
            # Maybe it's just "FLOW" with space, try rest
            parts = t.split(" ", 1)
            if len(parts) > 1:
                code = parts[1].strip()
            else:
                return False, (
                    "Send the code in this format: FLOW<code>\n"
                    "Example: FLOW12345"
                )
    ...
    # Legacy /myflow<code> support (deprecated but still works)
    if t.startswith(LEGACY_OTP_COMMAND_PREFIX):
        code = t[len(LEGACY_OTP_COMMAND_PREFIX):].strip()
        if not code.isdigit() or not (1 <= len(code) <= 8):
            return False, (
                "That doesn't look like a valid code. Digits only - "
                f"example: {LEGACY_OTP_COMMAND_PREFIX}12345"
            )
        return True, code
...
def extract_otp_from_command(text: str):
    ...
    if not text.startswith("/myflow"):
        return False, (
            f"Send the code in this format: /myflow<code>\n"
            f"Example: /myflow12345"
        )

    code = text[len("/myflow"):].strip()
