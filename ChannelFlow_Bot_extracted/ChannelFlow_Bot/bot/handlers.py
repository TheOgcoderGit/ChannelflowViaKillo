...
async def _start_connect_flow(message, user_id):
    ...
    if user_sessions.is_connected(user_id):
        await message.reply_text(i18n.t(user_id, "welcome.title", name="Deals"), reply_markup=main_menu)
        return
...
async def start(
    ...
    await update.message.reply_text(i18n.t(user_id, "home.welcome_back"), reply_markup=main_menu)
...
async def menu_handler(
    ...
    if text in MAIN_MENU_BUTTONS:
        ...
        if text == BTN_REWARDS:
            await _send_rewards(message, user.id)
            return
...
async def _send_rewards(message, user_id):
    ...
    lines = [
        ...
        f"👥 Invited Users: {referral_stats['total_invited']}",
        f"🏆 Milestones: {referral_stats['active_referrals']} active referrals",
        "",
        f"💰 Rewards Earned:",
        f"  Plan extensions: {referral_stats.get('plan_extensions', 0)} days",
        f"  Wallet credits: ₹{referral_stats.get('wallet_credits', 0):.2f}",
        "",
        f"🔗 Referral Link:",
        f"https://t.me/{bot_username}?start={referral_service.build_referral_code(user_id)}",
        "",
        f"📊 Leaderboard position: {referral_stats.get('leaderboard_position', 'N/A')}",
    ]
