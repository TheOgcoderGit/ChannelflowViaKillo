...
def mark_telegram_session_reconnect_required(telegram_id: int) -> bool:
    """Disable an unusable per-user Telegram session without deleting it.

    A session that cannot be decrypted or is no longer authorized must not be
    selected by either forwarding engine on the next restart.  Keeping the
    encrypted blob gives operators a narrow recovery/audit window, while the
    non-``connected`` status makes the owner go through the normal reconnect
    flow while retaining their external-account claim.  Only an explicit
    disconnect releases ownership; otherwise another user could take over an
    account merely because its stored session expired.

    Returns whether a connected row was transitioned.  This is deliberately
    conditional so a concurrent user disconnect cannot be overwritten.
    """
    conn = get_connection()
    try:
        cur = conn.execute(
...
def verify_telegram_session_reconnect_identity(cur, telegram_id: int, external_telegram_id: int) -> None:
    """Reject replacing a retained reconnect claim with another account.

    The caller must hold its write transaction before calling this helper.
    Keeping the check next to the ownership schema makes the reconnect rule
    explicit and independently testable.
    """
    existing = cur.execute(
        "SELECT external_telegram_id, status FROM user_telegram_sessions WHERE telegram_id=?",
        (telegram_id,),
    ).fetchone()
    if (
        existing
        and existing["status"] == "reconnect_required"
        and existing["external_telegram_id"] != external_telegram_id
    ):
        raise ValueError("reconnect identity does not match the retained ownership claim")
...
def _migrate_existing_schema(cur):
    ...
    # The bot user ID and the ID of the Telegram account authenticated through
    # Telethon are different identities.  Older installations only stored the
    # bot user ID, so add the external identity without invalidating existing
    # encrypted sessions.  The partial unique index below deliberately permits
    # historical rows without a verified external identity to be migrated.
    if not _column_exists(cur, "user_telegram_sessions", "external_telegram_id"):
        cur.execute("ALTER TABLE user_telegram_sessions ADD COLUMN external_telegram_id INTEGER")
...
def init_db():
    ...
    """)
    # Enforce the one-owner invariant in SQLite, rather than relying on a
    # check in the Telegram login handler that can race between updates.
    # Rebuild this index on startup so databases created before
    # ``reconnect_required`` existed gain the stronger predicate too.
    cur.execute("DROP INDEX IF EXISTS idx_active_external_telegram_owner")
    cur.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS idx_active_external_telegram_owner
        ON user_telegram_sessions(external_telegram_id)
        WHERE status IN ('connected', 'reconnect_required')
          AND external_telegram_id IS NOT NULL
    """)
    ...
    if cur.fetchone()[0] == 0:
        ...
        default_plans = [
            ...
            },
            {
                ...
                "daily_forward_limit": 500,
                "per_project_daily_forward_limit": 500,
                "requires_attribution": 0,
            ...
            },
            {
                ...
                "daily_forward_limit": 1500,
                "per_project_daily_forward_limit": 1500,
                "requires_attribution": 0,
