...
# ---- AI rewriting (BUG-007 fix) ----
    ...
    # Reserve one unit of the project's daily included quota for this
    # dispatch. The reservation is atomic and is shared across all
    # destinations for this source message, so multiple destinations do
    # not double-consume the daily quota.
    owner_id = route.get("owner_id")
    daily_reserved = False
    if owner_id is not None:
        daily_reserved = plan_service.reserve_daily_forward(owner_id, project_id)
